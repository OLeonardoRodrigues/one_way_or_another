defmodule OneWayOrAnother do
  ...
  defp binary(values, value, false, index) when length(values) > 1 do
    cond do
      Enum.at(values, div(length(values), 2)) == value ->
        binary(values, value, true, index + div(length(values), 2))

      Enum.at(values, div(length(values), 2)) > value ->
        binary(Enum.slice(values, 0..(div(length(values), 2) - 1)), value, false, index)

      Enum.at(values, div(length(values), 2)) < value ->
        binary(
          Enum.slice(values, (div(length(values), 2) + 1)..-1),
          value,
          false,
          index + div(length(values), 2) + 1
        )
    end
  end
